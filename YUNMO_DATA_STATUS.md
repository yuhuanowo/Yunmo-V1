...
> ⚠️ **【歷史文件 · 資料盤點過程】** 本檔為資料整理過程紀錄（含清廢檔、tokenizer 待辦等已完成事項）。**論文與最終定稿數字一律以 [`YUNMO_SPEC.md`](./YUNMO_SPEC.md) 為權威來源**；本檔數字雖已校準正確，但框架為「進行中」，僅供過程參照。

> 更新：2026-07-04 · 定位：繁體台灣特化 · 全功能小模型 · 基於 MiniMind v3 增量復現
...
- **打包實測（逐筆計，非抽樣）**：pretrain **2.35B tok**（229.8萬塊×1024）、SFT **3.87B tok**（189.2萬塊×2048），共 unique ~6.22B
  - （先前抽樣估 4.27B/5.85B 偏高，是被巨型離群記錄污染；打包逐筆才是真值）
- 36h 曝光：pretrain ~2.9–4.9 epoch、SFT ~2.1–3.6 epoch（依吞吐；時間封頂自適應）
- tok/param ≈ 35–59（近/超 Chinchilla 最優 20 → 餵得飽，非欠訓）
