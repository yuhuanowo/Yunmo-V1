...
from dataset.lm_dataset import PackedSFTDataset
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_size", type=int, default=64, help="batch size（Yunmo/PRO6000；seq2048，VRAM 有餘可上调）")
    parser.add_argument("--learning_rate", type=float, default=5e-5, help="初始学习率（Yunmo SFT 峰值）")
    ...
    parser.add_argument('--max_seq_len', default=2048, type=int, help="packing 塊長度（須≤打包時 --sft_seq）")
    parser.add_argument('--use_moe', default=0, type=int, choices=[0, 1], help="是否使用MoE架构（0=否，1=是）")
    parser.add_argument("--data_path", type=str, default="../dataset/yunmo_sft_ids.bin", help="SFT packed ids 二进位")
    parser.add_argument("--mask_path", type=str, default="../dataset/yunmo_sft_mask.bin", help="SFT packed loss-mask 二进位")
    parser.add_argument("--max_minutes", type=int, default=0, help="時間封頂(分鐘)，到時存檔停止；0=不限。雲端建議 sft ~1080")
